...
public final class Check {
    ...
    public static <T> List<P2<String, CheckResult>> check(final List<java.lang.Class<T>> c, final Rand r, final String... categories) {
        return join(c.map(new Func<java.lang.Class<T>, List<P2<String, CheckResult>>>() {
    ...
    public static <T> List<P2<String, CheckResult>> check(final java.lang.Class<T> c, final Rand r, final String... categories) {
        return join(clas(c).inheritance().map(new Func<Class<? super T>, List<P3<Property, String, Option<CheckParams>>>>() {
            public List<P3<Property, String, Option<CheckParams>>> f(final Class<? super T> c) {
                return properties(c.clas(), categories);
            }
        })).map(new Func<P3<Property, String, Option<CheckParams>>, P2<String, CheckResult>>() {
    ...
    public static <U, T extends U> List<P3<Property, String, Option<CheckParams>>> properties(final java.lang.Class<T> c, final String... categories) {
        ...
        final Array<P3<Property, String, Option<CheckParams>>> propFields = properties(array(c.getDeclaredFields()).map(new Func<Field, PropertyMember>() {
        ...
        //noinspection ClassEscapesDefinedScope
        final Array<P3<Property, String, Option<CheckParams>>> propMethods = properties(array(c.getDeclaredMethods()).map(new Func<Method, PropertyMember>() {
    ...
    private static <T> Array<P3<Property, String, Option<CheckParams>>> properties(final Array<PropertyMember> ms, final java.lang.Class<T> declaringClass, final String... categories) {
        final Option<T> t = emptyCtor(declaringClass).map(new Func<Constructor<T>, T>() {
        ...
        final Func<AnnotatedElement, Func<String, Boolean>> p = new Func<AnnotatedElement, Func<String, Boolean>>() {
            public Func<String, Boolean> f(final AnnotatedElement e) {
                return new Func<String, Boolean>() {
                    public Boolean f(final String s) {
                        final Func<Category, Boolean> p = new Func<Category, Boolean>() {
                            public Boolean f(final Category c) {
                                return array(c.value()).exists(new Func<String, Boolean>() {
        ...
        final Func<Name, String> nameS = new Func<Name, String>() {
            public String f(final Name name) {
                return name.value();
            }
        };

        return ms.filter(new Func<PropertyMember, Boolean>() {
        ...
        }).map(new Func<PropertyMember, P3<Property, String, Option<CheckParams>>>() {
