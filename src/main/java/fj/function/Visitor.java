...
public final class Visitor {
   ...
   * given default.
   */
  public static <A, B> B visitor(final List<Func<A, Option<B>>> visitors, final P1<B> def, final A value) {
   ...
   * given default.
   */
  public static <A, B> B nullableVisitor(final List<Func<A, B>> visitors, final P1<B> def, final A value) {
    return visitor(visitors.map(new Func<Func<A, B>, Func<A, Option<B>>>() {
      public Func<A, Option<B>> f(final Func<A, B> k) {
   ...
   */
  public static <A, B> Func<B, Func<A, B>> association(final List<P2<A, B>> x, final Equal<A> eq) {
    return curry(new Func2<B, A, B>() {
   ...
   * @return A function that can be applied to a default value (there is no association) and an associated key.
   */
  public static <A, B> Func<P1<B>, Func<A, B>> associationLazy(final List<P2<A, B>> x, final Equal<A> eq) {
    return curry(new Func2<P1<B>, A, B>() {
