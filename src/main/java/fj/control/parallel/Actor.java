...
public final class Actor<A> {
  ...
  private final Func<A, P1<Unit>> f;
  ...
  private Actor(final Strategy<Unit> s, final Func<A, P1<Unit>> e) {
    this.s = s;
    f = new Func<A, P1<Unit>>() {
   ...
   * @return A new actor that uses the given parallelization strategy and has the given side-effect.
   */
  public static <A> Actor<A> actor(final Strategy<Unit> s, final Func<A, P1<Unit>> e) {
   ...
   */
  public <B> Actor<B> comap(final Func<B, A> f) {
    return actor(s, new Func<B, P1<Unit>>() {
