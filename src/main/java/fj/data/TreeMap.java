...
public final class TreeMap<K, V> implements Iterable<P2<K, V>> {
   ...
   */
  public P2<Boolean, TreeMap<K, V>> update(final K k, final Func<V, V> f) {
   ...
   * @return A new tree map with the value for the given key transformed by the given function.
   */
  public TreeMap<K, V> update(final K k, final Func<V, V> f, final V v) {
  ...
  public P3<Set<V>, Option<V>, Set<V>> split(final K k) {
    final Func<Set<P2<K, Option<V>>>, Set<V>> getSome = Option.<V>fromSome().o(P2.<K, Option<V>>__2())
  ...
  public <W> TreeMap<K, W> map(final Func<V, W> f) {
    final Func<P2<K, Option<V>>, P2<K, Option<W>>> g = P2.map2_(f.mapOption());
    final Func<K, P2<K, Option<V>>> coord = flip(P.<K, Option<V>>p2()).f(Option.<V>none());
