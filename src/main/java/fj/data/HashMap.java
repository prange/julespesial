...
public final class HashMap<K, V> implements Iterable<K> {
   ...
   */
  public Func<K, Option<V>> get() {
    return new Func<K, Option<V>>() {
  ...
  public <A, B> HashMap<A, B> map(Func<K, A> keyFunction,
                                  Func<V, B> valueFunction,
  ...
  public <A, B> HashMap<A, B> map(Func<P2<K, V>, P2<A, B>> function, Equal<A> equal, Hash<A> hash) {
    return from(toStream().map(function), equal, hash);
  }

  public <A, B> HashMap<A, B> map(Func<P2<K, V>, P2<A, B>> function) {
    return from(toStream().map(function));
  }

  public <A> HashMap<A, V> mapKeys(Func<K, A> keyFunction, Equal<A> equal, Hash<A> hash) {
    return map(keyFunction, Function.<V>identity(), equal, hash);
  }

  public <A> HashMap<A, V> mapKeys(Func<K, A> function) {
    return mapKeys(function, Equal.<A>anyEqual(), Hash.<A>anyHash());
  }

  public <B> HashMap<K, B> mapValues(Func<V, B> function) {
  ...
  public void foreach(Func<P2<K, V>, Unit> function) {
    toStream().foreach(function);
  }

  public List<P2<K, V>> toList() {
    return keys().map(new Func<K, P2<K, V>>() {
