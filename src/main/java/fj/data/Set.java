...
public abstract class Set<A> implements Iterable<A> {
   ...
   */
  public final P2<Boolean, Set<A>> update(final A a, final Func<A, A> f) {
    return isEmpty()
           ? P.p(false, this)
           : tryUpdate(a, f).either(new Func<A, P2<Boolean, Set<A>>>() {
  ...
  private Either<A, P2<Boolean, Set<A>>> tryUpdate(final A a, final Func<A, A> f) {
    if (isEmpty())
      return right(P.p(false, this));
    else if (ord.isLessThan(a, head()))
      return l().tryUpdate(a, f).right().map(new Func<P2<Boolean, Set<A>>, P2<Boolean, Set<A>>>() {
    ...
    } else return r().tryUpdate(a, f).right().map(new Func<P2<Boolean, Set<A>>, P2<Boolean, Set<A>>>() {
      public P2<Boolean, Set<A>> f(final P2<Boolean, Set<A>> set) {
   ...
   */
  public static <A> Func<Set<A>, Func<A, Boolean>> member() {
    return curry(new Func2<Set<A>, A, Boolean>() {
   ...
   */
  public static <A> Func<A, Func<Set<A>, Set<A>>> insert() {
    return curry(new Func2<A, Set<A>, Set<A>>() {
   ...
   */
  public final <B> Set<B> map(final Ord<B> o, final Func<A, B> f) {
   ...
   */
  public final <B> B foldMap(final Func<A, B> f, final Monoid<B> m) {
    return isEmpty() ?
           m.zero() :
   ...
   */
  public final <B> Set<B> bind(final Ord<B> o, final Func<A, Set<B>> f) {
  ...
  /**
   * A first class function for {@link #union(fj.data.Set)}.
   * 
   * @return A function that adds all the elements of one set to another set.
   * @see #union(fj.data.Set)
   */
  public static <A> Func<Set<A>, Func<Set<A>, Set<A>>> union() {
    return curry(new Func2<Set<A>, Set<A>, Set<A>>() {
   ...
   */
  public final Set<A> filter(final Func<A, Boolean> f) {
   ...
   */
  public final Func<A, Func<Set<A>, Set<A>>> delete() {
    return curry(new Func2<A, Set<A>, Set<A>>() {
      public Set<A> f(final A a, final Set<A> set) {
        return set.delete(a);
      }
  ...
  /**
   * A first class function for {@link #intersect(fj.data.Set)}.
   * 
   * @return A function that intersects two given sets.
   * @see #intersect(fj.data.Set)
   */
  public static <A> Func<Set<A>, Func<Set<A>, Set<A>>> intersect() {
    return curry(new Func2<Set<A>, Set<A>, Set<A>>() {
      public Set<A> f(final Set<A> s1, final Set<A> s2) {
  ...
  /**
   * A first class function for {@link #minus(fj.data.Set)}.
   * 
   * @return A function that removes all elements of one set from another set.
   * @see #minus(fj.data.Set)
   */
  public static <A> Func<Set<A>, Func<Set<A>, Set<A>>> minus() {
    return curry(new Func2<Set<A>, Set<A>, Set<A>>() {
      public Set<A> f(final Set<A> s1, final Set<A> s2) {
        return s1.minus(s2);
      }
  ...
  public final int size() {
    final Func<A, Integer> one = constant(1);
  ...
  public static <A> Set<A> join(final Ord<A> o, final Set<Set<A>> s) {
    final Func<Set<A>, Set<A>> id = identity();
